...
# ALL SCHEDULE ------------------------------------------------------------------------------
	function get_by_date_all($stn, $date, $rows)
	{
		...
		LEFT JOIN ( SELECT * FROM daily_sch_dep_etd WHERE dsdetd_visible = \'y\' ORDER BY dsdetd_update_on DESC) AS etd ON sch.dsd_id = etd.dsdetd_dsd_id
	
		LEFT OUTER JOIN ( SELECT * FROM daily_sch_dep_adv WHERE dsdadv_visible = \'y\' ORDER BY dsdadv_update_on DESC) AS adv ON sch.dsd_id = adv.dsdadv_dsd_id
		
		LEFT OUTER JOIN ( SELECT * FROM daily_sch_dep_reg WHERE dsdreg_visible = \'y\' ORDER BY dsdreg_update_on DESC) AS reg ON sch.dsd_id = reg.dsdreg_dsd_id

		LEFT OUTER JOIN ( SELECT * FROM daily_sch_dep_atd WHERE dsdatd_visible = \'y\' ORDER BY dsdatd_update_on DESC) AS atd ON sch.dsd_id = atd.dsdatd_dsd_id
		
		LEFT OUTER JOIN ( SELECT * FROM daily_sch_dep_del WHERE dsddel_visible = \'y\' ORDER BY dsddel_update_on DESC) AS del ON sch.dsd_id = del.dsddel_dsd_id
		
		LEFT OUTER JOIN ( SELECT * FROM daily_sch_dep_gate WHERE dsdgate_visible = \'y\' ORDER BY dsdgate_update_on DESC) AS gate ON sch.dsd_id = gate.dsdgate_dsd_id

		LEFT OUTER JOIN ( SELECT * FROM daily_sch_dep_park WHERE dsdpark_visible = \'y\' ORDER BY dsdpark_update_on DESC) AS park ON sch.dsd_id = park.dsdpark_dsd_id

		LEFT OUTER JOIN ( SELECT * FROM daily_sch_dep_rotasi WHERE dsdrotasi_visible = \'y\' ORDER BY dsdrotasi_update_on DESC) AS rot ON sch.dsd_id = rot.dsdrotasi_dsd_id

		LEFT OUTER JOIN ( SELECT * FROM daily_sch_dep_remark WHERE dsdremark_visible = \'y\' ORDER BY dsdremark_update_on DESC) AS mark ON sch.dsd_id = mark.dsdremark_dsd_id
	
		WHERE dsd_stn = \'' . $stn . '\' 
		AND dsd_visible = \'y\' 
		AND dsd_date = \'' . $date . '\' 
		GROUP BY dsd_id DESC 
		ORDER BY dsd_std ASC 
		LIMIT ' . $rows . '  
		');
...
# ALL SCHEDULE BY AIRLINE -------------------------------------------------------------------
	...
	SELECT * FROM daily_sch_dep AS sch
LEFT JOIN ( SELECT * FROM daily_sch_dep_etd WHERE dsdetd_visible = \'y\' ORDER BY dsdetd_update_on DESC ) AS etd ON sch.dsd_id = etd.dsdetd_dsd_id
LEFT JOIN ( SELECT * FROM daily_sch_dep_adv WHERE dsdadv_visible = \'y\' ORDER BY dsdadv_update_on DESC ) AS adv ON sch.dsd_id = adv.dsdadv_dsd_id
LEFT JOIN ( SELECT * FROM daily_sch_dep_reg WHERE dsdreg_visible = \'y\' ORDER BY dsdreg_update_on DESC ) AS reg ON sch.dsd_id = reg.dsdreg_dsd_id
LEFT JOIN ( SELECT * FROM daily_sch_dep_atd WHERE dsdatd_visible = \'y\' ORDER BY dsdatd_update_on DESC ) AS atd ON sch.dsd_id = atd.dsdatd_dsd_id
LEFT JOIN ( SELECT * FROM daily_sch_dep_del WHERE dsddel_visible = \'y\' ORDER BY dsddel_update_on DESC ) AS del ON sch.dsd_id = del.dsddel_dsd_id
LEFT JOIN ( SELECT * FROM daily_sch_dep_gate WHERE dsdgate_visible = \'y\' ORDER BY dsdgate_update_on DESC ) AS gate ON sch.dsd_id = gate.dsdgate_dsd_id
LEFT JOIN ( SELECT * FROM daily_sch_dep_park WHERE dsdpark_visible = \'y\' ORDER BY dsdpark_update_on DESC ) AS park ON sch.dsd_id = park.dsdpark_dsd_id
LEFT JOIN ( SELECT * FROM daily_sch_dep_rotasi WHERE dsdrotasi_visible = \'y\' ORDER BY dsdrotasi_update_on DESC ) AS rot ON sch.dsd_id = rot.dsdrotasi_dsd_id
LEFT JOIN ( SELECT * FROM daily_sch_dep_remark WHERE dsdremark_visible = \'y\' ORDER BY dsdremark_update_on DESC ) AS mark ON sch.dsd_id = mark.dsdremark_dsd_id
	WHERE dsd_stn = \'' . $stn . '\' 
	AND dsd_airline_code = \'' .$ds_airline_daily . '\'  
	AND dsd_visible = \'y\' 
	AND dsd_date = \'' . $date . '\' 
	GROUP BY dsd_id DESC 
	ORDER BY dsd_std ASC 
